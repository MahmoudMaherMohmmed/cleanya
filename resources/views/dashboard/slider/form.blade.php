    ...
    <div class="breadcrumb-header justify-content-between">
        <div class="my-auto">
            <div class="d-flex">
                ...
                <span class="text-muted mt-1 tx-13 mr-2 mb-0">/ {{ $slider!=null ? __('dashboard.edit_slider') : __('dashboard.create_slider') }}</span>
            </div>
        </div>
    </div>
    <!-- breadcrumb -->
...
@section('content')
    ...
    <div class="row">
        <div class="col-lg-12 col-md-12">
            <div class="card">
                <div class="card-body">
                    <form method="post" action="{{$slider!=null ? route('sliders.update', $slider->id) : route('sliders.store')}}" enctype="multipart/form-data" data-parsley-validate="">
                        @csrf
                        ...
                        <div class="row row-sm">
                                                ...
                                                <div class="tab-content">
                                                    @foreach(LaravelLocalization::getSupportedLocales() as $localeCode => $properties)
                                                        <div class="tab-pane {{$loop->first ? 'active' : ''}}" id="tab-title-{{ $localeCode }}">
                                                            <input class="form-control" name="title[{{ $localeCode }}]" placeholder="{{ __('dashboard.title_placeholder') }}" value="{{$slider!=null ? $slider->getTranslation('title', $localeCode) : old('title[$localeCode]')}}" type="text">
                                                        </div>
                                                    @endforeach
                                                </div>
                                            </div>
                            ...
                            <div class="col-12">
                                <div class="form-group">
                                    ...
                                    <input type="file" name="image" class="dropify" data-default-file="{{$slider!=null && $slider->image!=null ? url($slider->image) : ''}}" data-height="200" />
                                </div>
                            </div>
                            ...
                            <div class="col-12">
                                <div class="form-group">
                                    ...
                                    <select class="form-control select2-no-search" name="status" require="">
                                        <option value="1" {{$slider!=null && $slider->status==1 ? 'selected' : ''}}> {{ __('dashboard.activate') }} </option>
                                        <option value="0" {{$slider!=null && $slider->status==0 ? 'selected' : ''}}> {{ __('dashboard.deactivate') }} </option>
                                    </select>
                                </div>
                            </div>

                            <div class="col-12"><button class="btn btn-main-primary pd-x-20 mg-t-10" type="submit">{{ $slider!=null ? __('dashboard.edit') : __('dashboard.save') }}</button></div>
                        </div>
                    </form>
                </div>
            </div>
